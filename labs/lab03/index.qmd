...
subtitle: "Conditional probability, independence, and simulation in R"
execute:
  eval: false
---
...
3. simulate an independent shooter; and
4. compare shooting streaks.
...
Now visualize the same conditional comparison using a proportional bar chart:
...
Because `position = "fill"` makes each bar total 1, the plot compares the **conditional distribution of the current shot within each previous-shot category**.

### Your turn: watch the denominator

Answer the following:

1. What is the observed value of $P(H_t \mid H_{t-1})$?
2. What is the observed value of $P(H_t \mid M_{t-1})$?
3. Are the two conditional probabilities exactly the same?
4. If shots were independent, what relationship would we expect between them and the overall probability of a hit?
5. What does the proportional bar chart show about the chance of a hit after a previous hit versus after a previous miss?
6. Does a difference in this one sample automatically prove dependence? Why not?
