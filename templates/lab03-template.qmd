...
If shots were independent, what relationship would we expect between these conditional probabilities and the overall probability of a hit?

> 

Does a difference in this sample automatically prove dependence? Explain.
...
Are $P(H_t \mid H_{t-1})$ and $P(H_t \mid M_{t-1})$ exactly equal in your simulation?
...
Compare the observed and simulated streak distributions. Are long streaks possible under independence?
...
Calculate the overall probability of a positive flag.
...
Calculate the probability that a case truly has the condition given a positive flag.
...
Why is $P(C \mid +)$ not the same as $P(+ \mid C)$?
...
Use R to calculate $P(\text{noncompliant} \mid \text{flagged})$.
...
Why is your answer not equal to 80%?

> 

Which probability is the base rate?
